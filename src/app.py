...
with st.sidebar:
    ...
    page = st.radio("", ["🏠 Home", "🔍 Analisis Query", "📍 Analisis Lokasi", 
                         "📈 Evaluasi Model", "💾 Model Management"])
...
if os.path.exists(dataset_path):
            ...
            with st.spinner("🔍 Menganalisis query dan mencari data..."):
                ...
                if is_shape_location_query and found_shape:
                    ...
                    if not city_stats:
                        ...
                        # Show top 5 cities as well
                        if len(city_stats) > 1:
                            st.markdown("### 🏙️ Top 5 Kota Lainnya untuk Bentuk Ini")
                            top_5 = city_stats[:5]
                            for i, (c, s, cnt, tot, pct) in enumerate(top_5, 1):
                                st.write(f"**{i}. {c.title()}, {s}** — {cnt} penampakan ({pct:.1f}% dari {tot} total)")
                        
                        # Show visualization
                        st.markdown("### 📊 Visualisasi Distribusi")
                        top_10_cities = city_stats[:10]
                        viz_df = pd.DataFrame(top_10_cities, columns=['City', 'State', 'Count', 'Total', 'Percentage'])
                        viz_df['Location'] = viz_df['City'].str.title() + ', ' + viz_df['State']
                        
                        fig = px.bar(
                            viz_df,
                            x='Location',
                            y='Percentage',
                            title=f'Top 10 Kota untuk Bentuk {shape.upper()}',
                            labels={'Percentage': 'Persentase (%)', 'Location': 'Kota'},
                            color='Percentage',
                            color_continuous_scale='viridis',
                            text='Percentage'
                        )
                    ...
                    handled = True
                    filtered_df = pd.DataFrame()  # Don't need filtered_df for this query type
    ...
    # NOTE: The previous "Model Training" page was removed per project requirements.
    # Training is no longer exposed in the app UI. Use external scripts to train models
    # and then place the saved model folders into the `saved_models/` directory so they
    # can be managed via the Model Management page.
        ...
        if 'models' in st.session_state and len(st.session_state.models) > 0:
            ...
            st.warning("⚠️ Belum ada model yang di-training. Silakan load model terlebih dahulu melalui 'Model Management'.")
        ...
        with col1:
            ...
            if saved_models:
                ...
                st.caption("Tempatkan folder model terlatih di `saved_models/` lalu refresh di 'Model Management'")
